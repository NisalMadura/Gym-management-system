  ...
  if ($result_payment->num_rows > 0) {
    ...
    if ($result_plan && $result_plan->num_rows > 0) {
     ...
     $pdf->Ln(8); 
      ...
      $sql_username = "SELECT username, memberno FROM users WHERE id = '$user_id'";
      $result_username = $conn->query($sql_username);

    if ($result_username->num_rows > 0) {
    $row_username = $result_username->fetch_assoc();
    $memberno = $row_username['memberno']; // Change here

    // Use $memberno instead of $user_id in the following line
    $pdf->Cell(0, 6, 'User Payment ID: ' . $memberno . ' - ' . $row_username['username'], 0, 1);
} else {
    $pdf->Cell(0, 6, 'User Payment ID: ' . $memberno . ' - Username Not Found', 0, 1);
}
      ...
      if (!empty($member_ids)) {
        ...
        foreach ($member_ids as $member_id) {
          $sql_member_info = "SELECT memberno, username FROM users WHERE id = '$member_id'";
          $result_member_info = $conn->query($sql_member_info);
      
          if ($result_member_info->num_rows > 0) {
              $row_member_info = $result_member_info->fetch_assoc();
              $memberno = $row_member_info['memberno'];
              $username = $row_member_info['username'];
    ...
    $pdf->Cell(0, 6, 'Package Amount: Rs ' . $amount, 0, 1);
      ...
      if (!empty($start_date) && DateTime::createFromFormat('Y-m-d', $start_date) !== false) {
        ...
        // SMS Sending Logic
        $message_sms = "You have enrolled with $package package and it will be expired on $expiry_date.your next installment amount is $next_amount. Thank You.";
